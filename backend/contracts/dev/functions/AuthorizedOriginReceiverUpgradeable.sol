...
/**
 * @notice Modified AuthorziedReceiver abstract for use on the FunctionsOracle contract to limit usage
 * @notice Uses tx.origin instead of msg.sender because the client contract sends message to the Oracle contract
 */
abstract contract AuthorizedOriginReceiverUpgradeable is Initializable {
    ...
    /**
     * @dev Triggers AuthorizedOriginReceiver usage to blok unauthorized senders
     * Requirements:
     * - The contract must be active
     */
    function activateAuthorizedReceiver() external validateAuthorizedSenderSetter {
        if (authorizedReceiverActive()) revert AlreadySet();
    ...
    function isAuthorizedSender(address sender) public view returns (bool) {
        if (!authorizedReceiverActive()) return true;
    ...
    function _validateIsAuthorizedSender() internal view {
        if (!isAuthorizedSender(tx.origin)) revert UnauthorizedSender();
    }
